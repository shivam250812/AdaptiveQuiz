...
// Neon Connection String (Provided by User)
const DATABASE_URL = process.env.DATABASE_URL || 'postgresql://neondb_owner:npg_LHj2uxmwA9dO@ep-fragrant-glade-aqng83ys-pooler.c-8.us-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require';
  ...
  try {
    ...
    if (count === 0) {
      ...
      for (const q of seedData) {
        await pool.query(
          `INSERT INTO questions (id, subject, difficulty, question, options, answer) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (id) DO NOTHING`,
          [q.id, q.subject, q.difficulty, q.question, q.options, q.answer]
        );
      }
      console.log("Database seeded successfully.");
    } else {
      console.log(`Neon database is ready. Found ${count} existing questions.`);
    }
  } catch (err) {
...
app.get('/api/questions', async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(`SELECT * FROM questions WHERE subject = $1`, [subject]);
...
app.post('/api/questions', async (req, res) => {
  ...
  try {
    await pool.query(
      `INSERT INTO questions (id, subject, difficulty, question, options, answer) VALUES ($1, $2, $3, $4, $5, $6)`,
      [id, subject, difficulty, question, JSON.stringify(options), answer]
    );
